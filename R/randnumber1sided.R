...
#'
#'
#' @return a vector of one-sided M-Wright distributed random numbers
#'
#'
#'
#' @references
...
#'
#' @export
rmwright1 <- function(n, nu, sc){
  U.1 <- runif(n);
  U.2 <- runif(n);
  U.3 <- runif(n);
  r.stab <- ( ( sin(nu*pi*U.2)*sin((1-nu)*pi*U.2)^(1/nu-1) )/ (  ( sin(pi*U.2)^(1/nu) )*abs(log(U.3))^(1/nu-1))  );
  return(sc*r.stab^(-nu));
}
